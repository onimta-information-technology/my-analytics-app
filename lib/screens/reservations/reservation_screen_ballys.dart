...
/// Ballys copy of `ReservationScreen`. Same four-tab workflow, but it reads the
/// Ballys reservation list ([reservationBallysProvider]) and hands off to the
/// Ballys new-reservation / view screens.
class ReservationScreenBallys extends ConsumerStatefulWidget {
  final bool hideAddButton;
  const ReservationScreenBallys({super.key, this.hideAddButton = false});

  @override
  ConsumerState<ReservationScreenBallys> createState() =>
      _ReservationScreenBallysState();
}

class _ReservationScreenBallysState
    extends ConsumerState<ReservationScreenBallys>
  ...
  Future<void> _loadReservationData() async {
    ...
    await ref.read(reservationBallysProvider.notifier).getReservationData();
    if (!mounted) return;
    setState(() => _isLoading = false);
  }

  Future<List<ReservationBallys>> _filterReservations(
    List<ReservationBallys> reservations,
  ) async {
  ...
  Widget build(BuildContext context) {
    final reservations = ref.watch(reservationBallysProvider);
  ...
  Widget _buildReservationList(List<ReservationBallys> reservations) {
    final fontSettings = ref.watch(fontSettingsProvider);

    return FutureBuilder<List<ReservationBallys>>(
              ...
              child: ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    // Every guest carried by this reservation, so a group
                    // booking is recognisable without opening the card.
                    if (reservation.guests.length > 1) ...[
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            '${reservation.guests.length} guests',
                            style: TextStyle(
                              fontSize: fontSettings.fontSize + 1,
                              fontWeight: fontSettings.fontWeight,
                    ...
                    // Approved / Checked / Rejected time
                    if (isApprovedOrRejected &&
                ...
                onTap: () async {
                  ...
                  ref
                      .read(selectedReservationBallysProvider.notifier)
                      .setSelectedBallysReservation(reservation);
